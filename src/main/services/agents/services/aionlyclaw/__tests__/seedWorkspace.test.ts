...
describe('seedWorkspaceTemplates', () => {
  ...
  it('creates directories and seeds templates when files do not exist', async () => {
    ...
    expect(mockedMkdir).toHaveBeenCalledWith(path.join('/workspace', 'memory'), { recursive: true })

    expect(mockedWriteFile).toHaveBeenCalledTimes(2)
    const writeCalls = mockedWriteFile.mock.calls.map((c) => c[0])
    expect(writeCalls).toContain(path.join('/workspace', 'SOUL.md'))
    expect(writeCalls).toContain(path.join('/workspace', 'USER.md'))

    // Verify template content
    const soulCall = mockedWriteFile.mock.calls.find((c) => c[0] === path.join('/workspace', 'SOUL.md'))
    expect(soulCall![1]).toContain('# Soul')
    expect(soulCall![1]).toContain('## Personality')

    const userCall = mockedWriteFile.mock.calls.find((c) => c[0] === path.join('/workspace', 'USER.md'))
  ...
  it('writes only missing files', async () => {
    ...
    expect(mockedWriteFile.mock.calls[0][0]).toBe(path.join('/workspace', 'USER.md'))
  })
})
